...
### Selection
- Clicking a **Default chip** switches it to **Applied state**, activating the filter.
- Clicking an **Applied chip** toggles it back to **Default**, removing the filter.

### Removal
- When in **Applied state**, a **“×” icon** appears allowing users to remove the filter directly.
- Clicking the **“×” icon** removes the filter immediately.

### Multiple Selection
- Multiple filter chips may be applied simultaneously unless restricted by the product’s filtering logic.

### Grouping
- Filter chips are typically displayed in **horizontal groups or rows above content lists or search results**.
