
# Foundations — Typography

**Category:** Documentation Root

# USAGE GUIDELINES

  
| Column 1 | Column 2 |  
| --- | --- |  
| Hierachy | - Use H4 for mobile screen titles (primary header).<br>- Use H5–H6 for section breaks (never jump from H4 → Body directly).<br>- Avoid skipping heading levels unless there is a clear structural reason. |  
| Content Length | - Use Subheading (18px) only for short, 1–4 line blocks.<br>- Use Body L for desktop reading experiences.<br>- Use Body S as the default mobile paragraph style. |  
| Capitalisation | Use ALL CAPS Heading only for short phrases such as swimlane titles or promo labels.<br>Avoid All Caps for<br>- Paragraphs<br>- Instructions<br>- Menu items<br>- Error messages |  
| Typographic Emphasis | Use Bold sparingly to highlight key actions or important terms.<br>
Do NOT:<br>- Bold entire paragraphs<br>- Mix too many weights in a single block<br>- Use italics for emphasis (support is inconsistent across platforms) |

